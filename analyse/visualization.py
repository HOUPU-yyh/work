    ...
    with PdfPages(args.output) as pdf:
        ...
        for event_num, ((run_id, event_id), raw_event_data) in enumerate(grouped_rawData):
            ...
            for i, track_idx in enumerate(real_track_indices):
                ...
                # 简化图例文本，将process信息放在最后，添加 PID, motherPID, turnId 信息
                legend_text = f'track {track_idx}: PID={pid}, mPID={mother_pid}, turnId={turn_id_str}, hits={hit_count}, layers={unique_layers}, process={process}'
                label_added = False
                ...
                # 按照scaledFltLen排序以获取顺序编号
                track_data_ordered = track_data.sort_values('scaledFltLen').reset_index(drop=True)
                # 创建索引映射：原始index -> 顺序编号
                index_to_order = {idx: order for order, idx in enumerate(track_data_ordered.index)}
                ...
                if not axial_hits.empty:
                    ...
                    # 标注顺序编号（根据开关控制）
                    if args.show_hit_numbers:
                        for idx, row in axial_hits.iterrows():
                            order_num = track_data_ordered[track_data_ordered['middleX'] == row['middleX']].index[0]
                            ax.text(row['middleX']/100, row['middleY']/100, str(order_num), 
                                   fontsize=5, ha='center', va='center', color=color, weight='bold')
                # 2. 绘制斜丝点（计算着火点并绘制空心圆）
                if not stereo_hits.empty:
                    ...
                    if all(col in stereo_hits.columns for col in required_cols):
                        try:
                            ...
                            if (np.abs(z_diff) > 1e-6).all():
                                ...
                                # 标注顺序编号（根据开关控制，使用计算后的着火点位置）
                                if args.show_hit_numbers:
                                    for idx, row in stereo_hits.iterrows():
                                        order_num = track_data_ordered[
                                            (track_data_ordered['middleX'] == row['middleX']) & 
                                            (track_data_ordered['middleY'] == row['middleY'])
                                        ].index[0]
                                        hit_x_val = (row['eastX'] + ((row['posZ'] - row['eastZ']) / (row['westZ'] - row['eastZ'])) * (row['westX'] - row['eastX'])) / 100
                                        hit_y_val = (row['eastY'] + ((row['posZ'] - row['eastZ']) / (row['westZ'] - row['eastZ'])) * (row['westY'] - row['eastY'])) / 100
                                        ax.text(hit_x_val, hit_y_val, str(order_num), 
                                               fontsize=5, ha='center', va='center', color=color, weight='bold')
                    ...
                    if not use_calculated_pos:
                        ...
                        # 标注顺序编号（根据开关控制，使用middleX/middleY位置）
                        if args.show_hit_numbers:
                            for idx, row in stereo_hits.iterrows():
                                order_num = track_data_ordered[
                                    (track_data_ordered['middleX'] == row['middleX']) & 
                ...
                # 绘制track的曲线（根据开关控制，分圈绘制，每圈用自己的参数）
                if args.show_track_curve:
                    # 若有turnId则分圈绘制，否则整体绘制
                    if 'turnId' in track_data.columns:
                        turn_groups = [
                            track_data[track_data['turnId'] == tid] 
                            for tid in sorted(track_data['turnId'].unique())
                        ]
                    else:
                        turn_groups = [track_data]

                    for t_hits in turn_groups:
                        if t_hits.empty:
                            continue
                        
                        # 为该圈分别计算圆心和半径（避免用整个track的参数）
                        pt_turn, r_turn, sign_turn, cx_turn, cy_turn = calculate_track_parameters(t_hits, use_turn_hint=False)
                        
                        if cx_turn is None or cy_turn is None or r_turn is None or r_turn <= 0:
                            continue
                        
                        track_x = t_hits['middleX'].values / 100
                        track_y = t_hits['middleY'].values / 100

                        track_angles = np.arctan2(track_y - cy_turn, track_x - cx_turn)
                        ...
                        curve_x = cx_turn + r_turn * np.cos(theta_smooth)
                        curve_y = cy_turn + r_turn * np.sin(theta_smooth)
                        plot_x = curve_x.copy()
                        plot_y = curve_y.copy()
                        ...
                        ax.plot(plot_x, plot_y, color=color, linestyle='-', 
                                alpha=0.9, linewidth=0.6, zorder=5)
...
def analyze_distance_to_tracks_distribution(args):
    ...
    for event_num, ((run_id, event_id), raw_event_data) in enumerate(grouped_rawData):
        ...
        # 第一步：识别好track并计算其自身hit点的距离（使用共享工具，按圈处理）
        ...
        # 第二步：获取电离process的hit点并计算到最近好track的距离
        ...
        if not ionization_hits.empty:
            ...
            for _, hit_data in ionization_hits.iterrows():
                dist, best_track, missing_geom = compute_hit_to_track_distance(hit_data, good_track_params)
                if missing_geom or dist is None or best_track is None:
                    continue
                ionization_distances.append(dist)
