...
from analysis import calculate_hit_relationships, analyze_hit_relationships, analyze_closest_distance_distribution, analyze_decay_hit_distribution
...
def main():
    ...
    parser.add_argument('--run-mode', required=True, 
                        choices=['raw', 'rec', 'distance', 'hit-relationships', 'distance-to-tracks', 'decay-hit-distribution'], 
                        ...
                        decay-hit-distribution: Analyze Decay process hit count distribution by motherPID (needs --input1)''')
    ...
    parser.add_argument('--show-momentum-arrows', action='store_true',
                        help='Show momentum direction arrows for eIoni, muIoni, hIoni process types (default: False)')
    parser.add_argument('--show-hit-numbers', action='store_true',
                        help='Show hit point sequence numbers ordered by scaledFltLen (default: False)')
    ...
    if args.run_mode in ['distance', 'hit-relationships', 'distance-to-tracks', 'decay-hit-distribution'] and args.input1 is None:
    ...
    if args.run_mode == 'raw':
        ...
        print(f"=== Running Decay hit distribution analysis ===")
        print(f"Input raw data: {args.input1}")
        print(f"Output PDF: {args.output}")
        print(f"Analyzing hit count distribution for Decay process by motherPID")
